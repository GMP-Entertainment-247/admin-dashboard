...
export default function DashboardHome() {
  ...
  const { data: activityLog, loading } = useFetch<{
    data: IActivityLog[];
    last_page: number;
  }>("/admin/activity-log", {
  ...
  const [lineChartDataKeys, setLineChartDataKeys] = useState([
    {
      ...
      color: "#00BF00",
      isActive: true,
      handleChange: handleLineChartDataKeyChange,
    },
    {
      label: "artists",
      color: "#AB1BB2",
  ...
  function handleLineChartDataKeyChange(keyLabel: string) {
    setLineChartDataKeys((prev) =>
      prev.map((k) =>
        k.label === keyLabel ? { ...k, isActive: !k.isActive } : k
      )
    );
  }
      ...
      <div className="grid grid-cols-2 max-[1200px]:grid-cols-1 gap-6 my-10">
        ...
        <div className="bg-white rounded-[16px] p-5">
          <div className="flex justify-between items-center">
            ...
            <Dropdown triggerText="This Month" options={[]} />
          </div>
          <PieChartComponent
            data={[
        ...
        <Table
              ...
              <Dropdown
                triggerText={
                  tableOrderOptions.find(
                    (item) => item.value === queryParam.get("order")
                  )?.label || "Most Recent"
                }
              ...
              <Dropdown
                triggerText={
                  tablePeriodOptions.find(
                    (item) => item.value === queryParam.get("period")
                  )?.label || "This Month"
                }
          ...
          rows={[
            ...
            },
            {
              header: "Description",
              view: (item) => (
                <p className="line-clamp-1 max-w-[700px]">
                  {item.description || "---"}
                </p>
              ),
            },
            ...
            },
            {
              header: "Time",
              view: (item) => (
                <span>{dayjs(item.created_at).format("hh:mma")}</span>
              ),
            },
          ]}
        />
